...
def get_prod_urls(session, category_name, category_link, in_myproduct_urls, page_num = 1):
    ...
    if config.categories_id_map:
        config.params['coll'] = config.categories_id_map[category_name.lower()]
    if config.prod_store_api_base_url:
        category_link = config.prod_store_api_base_url
    category_prod_page_link = category_link
    response = session.post(category_prod_page_link, params=config.params, headers=config.headers, data=config.data)

    # response = session.post(category_prod_page_link, headers=config.headers, data=config.data)
...
def get_variants(json_variant_obj, session, prod_item_num, prod_color_tag, prod_size_tag, variant_url_tag):
    ...
    variant_url = variant_url_tag.get('onclick').split(',')[1].replace("'", '').strip() + '.jpg'
...
def start():
    ...
    setup_logging()
    print(in_master_json_file_path)
