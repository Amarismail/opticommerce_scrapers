def visit_html_page(session, url, headers, cookies=None, params=None, requests_data=None, verify=True):
    ...
    try:
        ...
        if "CERTIFICATE_VERIFY_FAILED" in str(e):
            ...
            logging.info("trying again without certificate")
...
def request_website_api(session, url, headers, cookies=None, params=None, requests_data=None, verify=True):
    import json
...
def check_image_url(url, headers=None, verify=False):

    import requests
    import logging

    try:
        response = requests.head(url, headers=headers, timeout=20, verify=verify)
...
def browser_login_to_website(page, session):
    from utils.utils import delay
    import constants
    import config

    page.goto(config.login_url)
    page.fill('#username', constants.user_name)
    page.fill('#password', constants.password)
