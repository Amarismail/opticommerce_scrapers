...
def get_prod_final_image(json_obj, variant_img_url):
    ...
    check = check_image_url(variant_img_url, headers=config.headers)
    if not check:
        return json_obj
...
def get_product_type(json_prod_obj, prod_name, prod_category, prod_material=None):

    if prod_material:
        json_prod_obj['$iProduct_Material'] = prod_material
    ...
    for prod_type_key, keyword_to_update in mappings.prod_types.items():
        if prod_type_key in prod_category.lower() or prod_type_key in prod_name.lower():
            json_prod_obj['$iProduct_Type'] = keyword_to_update
            break

    return json_prod_obj
...
def basic_product_details(in_page, json_prod_obj, prod_category):
    ...
    json_prod_obj['$iProduct_Gender'] = prod_category
    json_prod_obj = get_product_type(json_prod_obj, prod_name, prod_category)
...
def get_product_details(session, in_page, in_product_url, master_json, prod_category): 
    ...
    for variant_color_tag, variant_url_tag in variant_info_tags_map:
        ...
        for prod_size_tag in prod_variant_sizes:
            ...
            try:
                int(json_variant_obj['$iFrame_Bridge'])  # Attempt to convert to int
                int(json_variant_obj['$iFrame_Temple'])  # Attempt to convert to int
                int(json_variant_obj['$iLens_Size_w'])  # Attempt to convert to int
            except (ValueError, TypeError):
                logging.info("Frame size value is not convertible to an integer.")
                continue
...
def start():
    ...
    playwright, browser, context, in_page = initialize_browser(constants.website_url, headless=True)
    if config.login_url:
    ...
    for prod_category, in_myproduct_urls in reversed(list(in_myproduct_urls_dict.items())):
        try:
