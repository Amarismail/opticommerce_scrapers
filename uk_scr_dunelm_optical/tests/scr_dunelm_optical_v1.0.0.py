...
def map_website_keywords_to_json(json_obj, prod_all_specs):
    ...
    for spec in prod_all_specs:
        ...
        for splitter in splitters:
            if splitter in spec.text:
                spec_key = spec.get_text(strip=True).split(splitter)[0].lower()
                spec_value = spec.get_text(strip=True).split(splitter)[1].lower()
            # implemented else for prodesign denmark but can be achieved using \n
            # else:
...
def get_variants(json_variant_obj, session, prod_item_num, prod_color_tag, prod_size_tag, variant_url_tag):
    ...
    variant_url = variant_url_tag.get('href') or variant_url_tag.get('src') or variant_url_tag.get('data-zoomimg')
...
def get_product_details(session, in_page, in_product_url, master_json, prod_category): 
    ...
    variant_url_tags = in_page.xpath(constants.variant_urls_selector)
