...
def get_variants(json_variant_obj, product_name, variants_tag, stock_tag):
    ...
    if variant_img:
        variant_img_url = variant_img.get_attribute('src').replace('_s.jpg', '.jpg').replace('_s.jpeg', 'jpeg')
        json_variant_obj['$iFrame_IMG_Mark'] = "1"
        if variant_img_url == "" or constants.no_prod_img_url_key.lower() in variant_img_url.lower():
            variant_img_url = 'https://masterdb.co.uk/images/awaiting.jpg'
...
def get_product_details(in_page, in_product_url, master_json, prod_type_title):
    ...
    if brand_name == 'White':
        ...
        brand_name = 'Private Label White'
    elif brand_name == 'Black':
        json_prod_object['$iProduct_Gender'] = 'Man'
        brand_name = 'Private Label Black'
    else:
...
def main():
    ...
    playwright, browser, context, in_page = initialize_browser(constants.website_url, localhost=True)
    # context.clear_cookies()
    # in_page.goto("https://privatelabel.partners")
