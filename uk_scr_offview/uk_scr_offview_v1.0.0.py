...
def get_categories_prod_url(session, context, categories_dict, sub_categories_dict):
    total_products = 0
    in_myproduct_urls_dict = {}
    for category_name, category_link in categories_dict.items():
        for sub_cat_name, sub_cat_link in sub_categories_dict.items():
            print('==================================')
            logging.info(f'scraping category: {category_name} & sub cat {sub_cat_name}....')
            ...
            # in_myproduct_urls_dict[category_name][sub_cat_name] = in_myproduct_urls
            in_myproduct_urls_dict[category_name] = {sub_cat_name: in_myproduct_urls}
            total_products = total_products + len(in_myproduct_urls)
    
    return in_myproduct_urls_dict, total_products

def get_prod_urls(session, context, category_name, category_link, sub_cat_name, in_myproduct_urls, page_num = 1):
    
    category_prod_page_link, params = config_store_page_request(category_name, category_link, sub_cat_name, page_num)
    print('-------------------------------')
...
def get_variants(json_var_obj, session, prod_item_num, prod_color_tag, prod_size_tag, variant_url_tag):
    ...
    if config.imbedded_variants_json_data:
        variant_url = reduce(getitem, constants.variant_page_img_url_selector, variant_url_tag)
...
def basic_product_details(in_page, json_prod_obj, prod_category, prod_sub_category):
    ...
    json_prod_obj["$iProduct_Gender"] = prod_sub_category # added for offview

    return json_prod_obj, prod_item_num


def get_product_details(session, in_page, in_product_url, master_json, prod_category, prod_sub_category): 
    ...
    json_prod_obj, prod_item_num = basic_product_details(in_page, json_prod_obj, prod_category, prod_sub_category)
    logging.info(f"Scraped {in_product_url}")
    
    prod_all_specs = in_page.select(constants.prod_all_specs_selector)
    json_prod_obj = map_website_keywords_to_json(json_prod_obj, prod_all_specs, mappings.website_keyword_to_json_mapping, mappings.shapes)
    print(json_prod_obj)
    # return
...
def start():
    ...
    in_myproduct_urls_dict, total_products = get_categories_prod_url(session, context, categories_dict, config.sub_categories_mapping)
    logging.info(f"in_myproduct_urls_dict: {in_myproduct_urls_dict}")
    logging.info(f"Total product url {total_products}")

    for prod_category, prod_sub_category_dict in in_myproduct_urls_dict.items():
        for prod_sub_category, in_myproduct_urls in prod_sub_category_dict.items():
            try:
                logging.info(f"Scraping {prod_category} & sub cat: {prod_sub_category} .....")
                ...
                for in_product_url in in_myproduct_urls:
                    ...
                    try:
                        ...
                        resp_status_code, in_page = visit_html_page(session, in_product_url, config.headers)
                    try:
                        master_json = get_product_details(session, in_page, in_product_url, master_json, prod_category, prod_sub_category)
                    except:
