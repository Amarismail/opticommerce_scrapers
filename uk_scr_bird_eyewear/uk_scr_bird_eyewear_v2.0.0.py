...
def get_variants(json_var_obj, session, prod_item_num, prod_color_tag, prod_size_tag, variant_url_tag):
    ...
    variant_code = None
    variant_color = prod_color_tag.get_text(strip=True) if prod_color_tag else None
    variant_size = prod_size_tag.get_text(strip=True) if prod_size_tag else None
    variant_url = variant_url_tag

    if config.imbedded_variants_json_data  and isinstance(variant_url_tag, dict):
    ...
    json_var_obj['$iFrame_Code'] = variant_code or ''
...
def get_product_details(session, in_page, in_product_url, master_json, prod_category): 
    ...
    if config.imbedded_variants_json_data:
        try:
            variant_url_tags = json.loads(variant_url_tags[-1].get_text(strip=True))
        except:
